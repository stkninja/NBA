...
public class HistoryData {
	...
	public HistoryData(String name) throws IOException{
		bl = new TeamBL();
		list = bl.getAllSeasonTeam(name);
		this.chart1();
		this.chart2();
	}
	...
	public void chart2() throws IOException {  
	 ...
	 private CategoryDataset getDataSet1() {
		    ...
		    for(int n = 0;n < list.size();n++){
		    	ds.addValue(list.get(n).scores, "单赛季场均得分", list.get(n).season);
		    }
	        return ds;  
	    }
	 private CategoryDataset getDataSet2() {
		    DefaultCategoryDataset ds = new DefaultCategoryDataset();  
		    for(int n = 0;n < list.size();n++){
		    	ds.addValue(list.get(n).shootingHitRate*100, "单赛季命中率%", list.get(n).season);		    	
		    }
	        return ds;  
	    } 
